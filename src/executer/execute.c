...
int	ft_executer(t_mini **mini)
{
	...
	else
	{
		while (exec->aux && i < exec->total_cmnds)
		{
			...
			if (((exec->aux->broken == 0 && exec->aux->next && exec->aux->next->broken == 0) || (!exec->aux->next && exec->aux->broken == 0))//En caso de que estemos en un broken pipe o el siguiente sea un broken pipe no se ejecuta el comando
			&& ((exec->aux->full_cmd && ft_is_parent(exec->aux->full_cmd[0]) == 0 && ft_is_builtin(exec->aux->full_cmd[0]) == 1)//cd se ejecuta en el proceso padre
